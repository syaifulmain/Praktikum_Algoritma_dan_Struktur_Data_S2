...
//    }
    ...
    public int findBinarySearcch(int cari, int left, int right, SortBy sortBy) {
        int mid;
        ...
        if (right >= left) {
            ...
            if (sortBy == SortBy.ASC) {
                if (listBuku[mid].kodeBuku > cari) {
                    return findBinarySearcch(cari, left, mid - 1, sortBy);
                }
                return findBinarySearcch(cari, mid + 1, right, sortBy);
            } else {
                if (listBuku[mid].kodeBuku < cari) {
                    return findBinarySearcch(cari, left, mid - 1, sortBy);
                }
